...
class walkthru_gui(xbmcgui.WindowXMLDialog):
	...
	def __init__(	self, 
		...
		self.timezones = osmc_system.get_timezones()
		...
		# get the languages
		self.languages = osmc_system.get_languages()
		...
		Vero = osmc_system.is_Vero()
		log('Hardware is Vero: %s' % Vero)
		if not Vero:
	...
	def start_checking_for_internet(self, networking_instance):
		...
		if not self.testing:
			self.internet_checker = osmc_netcaller.Networking_caller(self, self.net_call)
		else:
			self.internet_checker = osmc_netcaller.mock_Networking_caller(self, self.net_call)
	...
	def exit_proceedure(self):
		...
		osmc_system.delete('/tmp/NO_UPDATE')
