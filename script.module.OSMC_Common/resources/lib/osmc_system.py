...
def retrieve_modules(known_modules_order=None, exclude=[]):
	...
	for addon_path in addon_paths:
		...
		folders  = [item for item in os.listdir(addon_path) if os.path.isdir(os.path.join(addon_path, item))]
		...
		for folder in folders:
			...
			MyOSMC_Modules[folder] = setting_instance

	MyOSMC_Modules = MyOSMC_Modules.items()
